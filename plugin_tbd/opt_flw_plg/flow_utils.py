...
import torch
import argparse
from opt_flw_plg.RAFT.raft import RAFT
from opt_flw_plg.RAFT.utils.utils import InputPadder
...
flowmodels_path_for_plugin="../models/FlowModels/"
...
def RAFT_estimate_flow(frame1, frame2, device='cuda'):
  ...
  model_path = flowmodels_path_for_plugin + '/raft-things.pth'
  ...
  if RAFT_model is None:
    args = argparse.Namespace(**{
        'model': flowmodels_path_for_plugin + '/raft-things.pth',
...
def compute_diff_map(next_flow, prev_flow, prev_frame, cur_frame, prev_frame_styled, args_dict):
  ...
  alpha_mask = np.maximum.reduce([diff_mask_flow * args_dict['occlusion_mask_flow_multiplier'], \
  ...
  # Dump all flow data
  img = Image.fromarray((alpha_mask*255).astype(np.uint8))
  img.save("../output/ttv/opt_flw/alpha_mask.png")
  
  img = Image.fromarray((warped_frame_styled).astype(np.uint8))
  img.save("../output/ttv/opt_flw/warped_frame.png")
