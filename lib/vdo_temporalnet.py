...
def split_video_into_frames(video_path, frames_dir):
    ...
    print("Input video dimension ", image.shape)
    count = 0
    while success:
        frame_path = os.path.join(frames_dir, f"frame{count:04d}.png")
        image = cv2.resize(image, dsize=(1024, 1024), interpolation=cv2.INTER_LANCZOS4)
...
parser.add_argument('--prompt', default='a hamburber witth all lettuce ', help='the stable diffusion prompt')
...
for i, frame_file in enumerate(frame_files):
    ...
    canny_image = cv2.Canny(canny_image, 25, 150)
    ...
    # Generate image
    image = pipe(
       prompt, num_inference_steps=50, generator=generator, image=[last_generated_image, canny_image], controlnet_conditioning_scale=[0.6, 0.7]
    ...
    # print("Input image shapes ", canny_image.size, last_generated_image.size, image.size)
